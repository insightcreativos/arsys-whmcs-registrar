...
class WHMCS_Service implements WHMCSService_Interface {
		...
		if ( $response['result'] == 'error' ) {
			throw new Exception(
				...
				( array_key_exists( 'message',
					$response ) ? $response['message'] : 'Something went wrong with localAPI' )
			);
		}
	...
	/**
	 * Check if a TLD exists and is configured to work with Arsys.
	...
	public function tldExists( $tld ) {
		...
		$checkTLD = Capsule::table( 'tbldomainpricing' )->where( [
			'extension' => $tld,
			'autoreg'   => 'arsys'
		] )->first();
	...
	public function createOrder( $clientId ) {
		...
		$insert = Capsule::table( 'tblorders' )->insert( [
			...
			'invoiceid' => 0,
			'status'    => 'Active',
			'notes'     => 'Created automatically by Arsys Registrar Module v' . $this->getApp()->getVersion() . ' on ' . date( 'm-d-Y H:i:s' )
		] );
	...
	public function createDomain( $orderId, $clientId, $response ) {
		...
		if ( is_object( $domainPricing ) ) {
			$pricing = Capsule::table( 'tblpricing' )->where( [
				'type'  => 'domainrenew',
		...
		$domain->additionalnotes    = 'Created automatically by Arsys Registrar Module v' . $this->getApp()->getVersion() . ' on ' . date( 'm-d-Y H:i:s' );
		$domain->synced             = 0;
	...
	public function getCustomFieldsByType( $type ) {
		$customFields = Capsule::table( 'tblcustomfields' )->where( 'type',
			$type )->orderBy( 'fieldname' )->pluck( 'fieldname' );
